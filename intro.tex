...
There is a combination of factors explaining this slow down, among which the application of energy policies in data centres.

This is why we think the attention of researchers in the field of Energy Management in Clouds should shift from the paradigm of "consume less" to the paradigm of "consume better".
As important energy consumers, it is important the energy management and policies of data centres prioritize the consumption of renewable energies over brown energies.
...
In this paper we start to analyse how a PaaS on top of IaaS may change the landscape.
We think that a PaaS layer brings great potentialities for a better energy management in data centres, because it centralizes and standardizes the scalable deployment of applications.
Typically, an application manager will issue a command to a PaaS framework (such as CloudFoundry) to scale up or down and application, according to predefined patterns.
Some frameworks allow to automatize this process: CloudFoundry, for example, provides a languages for auto-scaling.
This language will define KPI and thresholds that will trigger the scaling operations.
For example, in the case of a 3-tier web server application, it is possible to describe that if the number of page views per minute goes over a threshold, a new front-end VM should be launched.

However, we think that a piece is missing: indeed the framework still have no way to lower the workload when there is no renewable energy, for instance.
We believe that some part of the flexibility provided to the applications by the PaaS layer should be transferee to an intermediate component called the "autoscaler".
...
The paper will also discuss how the combination of IaaS and PaaS consolidation techniques can improve energy saving and cleverly applied to reduce CO2 energy consumption scaling up and down applications following the curve of green energy availability.

In this paper we propose the concept of Service Flexibility Agreement (SFA). 
The SFA is and extended Service Level Agreement (SLA): it includes a description of the flexibility of an application.
