...
Typically, an application manager will issue a command to a PaaS framework to scale up or down and application, according to predefined patterns.
Some frameworks allow to automatize this process: Cloudify\footnote{http://www.gigaspaces.com/cloudify-cloud-orchestration/overview}, for example, provides a languages for auto-scaling.
...
However, when it comes to the adaptation of the applications workload to the availability of the renewable energies, we think that a piece is missing: indeed the framework still have no way to lower the workload when there is no renewable energy, for instance.
...
The paper also discusses how the combination of IaaS and PaaS consolidation techniques can improve energy saving and cleverly applied to reduce CO2 energy consumption scaling up and down applications following the curve of green energy availability.
