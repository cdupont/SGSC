...
This language will define KPIs \ms{(Key Performance Indicators)} and thresholds that will trigger the scaling operations.
...
However, when it comes to the adaptation of the applications workload to the availability of the renewable energies, we think that a piece is missing: indeed the framework still has no way to lower the workload when there is no renewable energy, for instance. \ms{not clear}
...
However, in the context of flexible applications, this is not enough: some applications can accept to have a temporarily reduced performance or shifted activity. \ms{also flexibility provides maximum level of performance}
The SFA defines a simple interface to describe this flexibility between an application and the PaaS framework. 
This will allow an energy-aware PaaS framework to make better decisions and trace-offs. \ms{more description needed: e.g. make better decision for apps consolidation, etc.}

The paper also discusses how the combination of IaaS and PaaS consolidation techniques can improve energy saving and cleverly applied to reduce CO2 energy consumption scaling up and down applications following the curve of green energy availability. \ms{not sure about green discussion, but that make sense if we provide some more description and intuition}
