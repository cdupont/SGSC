% !TEX root =  main.tex
\ms{are we going to talk about green energy? or only energy management at PaaS. }
...
Those researches had an impact (probably together with other external factors): a recent study showed that the energy consumption of datacentres is actually less than what was previously predicted. \ms{not clear the message of this sentence}
This calls for a qualitative change: instead of aiming at consuming less in data centre, we argue that a new research goal should be to prioritize the consumption of green energies; i.e. "consume better". \ms{I think we are not addressing this aspect, so we need to remove if not...; the same comment for the next sentence}
